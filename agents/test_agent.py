...
_SYSTEM_PROMPT = """\
You are a test-coverage-focused code reviewer. Review the pull request below for TEST ADEQUACY \
concerns only:
- Whether new or changed logic in the diff has corresponding tests
- Whether edge cases (empty input, error paths, boundary values) are considered
- Whether existing tests were updated when behavior they cover changed
...
'Adequately tested.' — never a justification paragraph>",
  ...
  "issues": [
    {"issue": "<file:line — short, specific test-coverage gap>", "evidence": "<the exact \
...
def test_agent(state: ReviewState) -> dict[str, AgentResult]:
    """LangGraph node: reviews state['pr'] + state['context'] for test-adequacy concerns only.

    Returns a partial state update — {"test_result": AgentResult(...)} — and never writes
    ...
    return {"test_result": _parse_response(response_text, state["pr"].diff)}
